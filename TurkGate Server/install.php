  ...
  <?php
    ...
    if(isset($_POST['submit'])) {
      ...
      if (empty($database_host) || empty($database_username) || empty($database_username) || empty($database_password)) {
        ...
        if(!$connection) {
          die("<h1>TurkGate</h1><h2>Installation</h2><p>Error connecting to database. " . mysql_error() . ". See your system administrator.</p><p><a href='javascript:history.back()'>Go back</a></p>" . $footer);
        } else {
		  // Create the TurkGate configuration file
	      $configFileName = "turkgateConfig.php";
	      $configFileHandle = fopen($configFileName, 'w') or die("<h1>TurkGate</h1><h2>Installation</h2><p>Error creating config file. " . mysql_error() . ".</p><p>See administrator.</p>" . $footer);
	      ...
	      if (mysql_query("CREATE DATABASE IF NOT EXISTS " . $database_name, $connection)) {
		    // Select the database
		    if(mysql_select_db('TurkGate', $connection)) {
		  	  // Create the table
			  ...
			  if(mysql_query($sql, $connection)) {
			    ...
			    die("<p>Error creating table: " . mysql_error() . ". See administrator.</p><p><a href='javascript:history.back()'>Go back</a></p>" . $footer);
			  }
		    } else {
			  die("<p>Error selecting database: " . mysql_error() . ". See administrator.</p><p><a href='javascript:history.back()'>Go back</a></p>" . $footer);
		    }
		  } else {
		    die("<p>Error creating database. (Might it already exist?): " . mysql_error() . ". See administrator.</p><p><a href='javascript:history.back()'>Go back</a></p>" . $footer);
		  }
